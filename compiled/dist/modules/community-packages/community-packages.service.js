...
let CommunityPackagesService = class CommunityPackagesService {
    ...
    async checkForMissingPackages() {
        ...
        if (reinstallMissing) {
            ...
            const vettedPackages = await (0, community_node_types_utils_1.getCommunityNodeTypes)(environment);
            const checksums = new Map();
            for (const p of vettedPackages) {
                const versionMap = new Map();
                versionMap.set(p.npmVersion, p.checksum);
                for (const v of p.nodeVersions ?? [])
                    versionMap.set(v.npmVersion, v.checksum);
